...
// Device-compatible work plan for one Q chunk. A window whose K-chunk-rounded
// predecessor halo fits in one local slab touches at most the Q-owned region and
// its predecessor, so two fixed ranges cover the chunked layout without a
// dynamic container.
