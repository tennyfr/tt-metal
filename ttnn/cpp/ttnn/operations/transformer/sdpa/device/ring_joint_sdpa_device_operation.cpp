...
void RingJointSDPADeviceOperation::validate_on_program_cache_miss(
    ...
    if (args.has_sliding_window()) {
        ...
        const uint32_t sliding_window_size = args.sliding_window_size.value();
        const uint32_t halo_tokens = sliding_halo_token_count(sliding_window_size, k_chunk_size);
        TT_FATAL(
            halo_tokens > 0,
            "Ring sliding-window attention currently requires a non-empty predecessor halo; got window {}. "
            "A one-token local-only window requires a separate no-halo path.",
            sliding_window_size);
        TT_FATAL(
        ...
        TT_FATAL(
            !args.is_balanced,
            "Chunked ring sliding-window attention has a bounded, full window per Q and folds local/halo K/V into "
            "one synthetic pass. The dense-ring balanced causal/zigzag schedule halves or skips ring-phase work and "
            "would omit valid window chunks. Pass is_balanced=false.");
        ...
        TT_FATAL(
            ...
            "Ring sliding-window attention uses a one-hop predecessor halo. Window {} requires a {}-token "
            "K-chunk-rounded halo, which exceeds the per-device Q slab {}. Use a window whose halo fits one "
            "local slab; wider windows require a multi-hop halo implementation.",
            sliding_window_size,
            halo_tokens,
            N_local_q);
    } else {
