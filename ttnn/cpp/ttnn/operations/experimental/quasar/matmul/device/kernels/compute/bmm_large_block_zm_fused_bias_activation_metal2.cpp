...
// RUNTIME ARGS
    ...
    for (std::uint32_t b = 0; b < batch; b++) {
        if constexpr (get_batch_from_reader) {
            ...
            UNPACK(is_batch_valid = (bool)mailbox_read(ckernel::ThreadId::UnpackThreadId);)
            MATH(is_batch_valid = (bool)mailbox_read(ckernel::ThreadId::UnpackThreadId);)
            PACK(is_batch_valid = (bool)mailbox_read(ckernel::ThreadId::UnpackThreadId);)
            if (!is_batch_valid) {
                continue;
            }
        }
...
// Configure packer once for pack out without Bias
                    ...
                    // never reload when with bias, bias uses interim buffer
                    enable_reload = false;
#else
