    ...
    ttnn::bind_function<"fused_rms_minimal">(
        ...
        R"doc(
            ...
            - stats: optional. Backs the op's internal all-gather circular buffer. When omitted
              ...
              (1, 1, 32, num_devices_on_cluster_axis * 32) on a single core, and its dtype must be
              consistent with the compute config accumulation (FLOAT32 when fp32_dest_acc_en is
              enabled, otherwise BFLOAT16). A stats buffer whose tile width does not equal the
              cluster-axis device count is rejected.
