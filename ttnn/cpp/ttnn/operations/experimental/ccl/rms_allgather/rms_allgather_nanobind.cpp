    ...
    ttnn::bind_function<"fused_rms_minimal">(
        ...
        R"doc(
            ...
            - stats: optional. Backs the op's internal all-gather circular buffer. Prefer omitting it
              ...
              ``cluster_axis``); a narrower buffer is rejected. Its dtype must be consistent with the
              compute config accumulation (FLOAT32 when fp32_dest_acc_en is enabled, else BFLOAT16).
