...
void kernel_main() {
    ...
    constexpr std::uint32_t cb_id_in0 = dfb::in0;

    // ublocks size defined in tiles
    constexpr std::uint32_t onetile = 1;
    DataflowBuffer cb_id_in0_obj(cb_id_in0);
    std::uint32_t src0_tile_bytes = cb_id_in0_obj.get_entry_size();
    ...
    constexpr std::uint32_t cb_id_attn = dfb::fused_attn;
    DataflowBuffer cb_id_attn_obj(cb_id_attn);
    std::uint32_t mask_tile_bytes = cb_id_attn_obj.get_entry_size();
    ...
    {
        constexpr std::uint32_t cb_max_scaler = dfb::max_scaler;
        constexpr std::uint32_t cb_sum_scaler = dfb::sum_scaler;
