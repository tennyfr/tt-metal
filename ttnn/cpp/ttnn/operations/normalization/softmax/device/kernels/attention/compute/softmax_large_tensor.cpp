...
void apply_fused_scale_mask(
    std::uint32_t cb_in, std::uint32_t cb_fused_scale_mask, std::uint32_t cb_out, std::uint32_t cb_length_t, std::uint32_t blk);
void apply_fused_attn_mask(
    std::uint32_t cb_in, std::uint32_t cb_fused_attn_mask, std::uint32_t cb_out, std::uint32_t cb_length_t, std::uint32_t blk, bool do_mask);
void pad_input(std::uint32_t cb_in, std::uint32_t cb_out, std::uint32_t cb_length_t, std::uint32_t blk);
void exp_cb(std::uint32_t cb_in, std::uint32_t cb_out, std::uint32_t cb_max, std::uint32_t cb_length_t, std::uint32_t blk);

template <PoolType reduce_type, std::uint32_t cb_in_id, std::uint32_t cb_scaler_id, std::uint32_t cb_prev_out_id, std::uint32_t cb_out_id>
void reduce_cb(bool use_prev_reduce, std::uint32_t cb_length_t);
void apply_recip(std::uint32_t cb_in, std::uint32_t cb_recip, std::uint32_t cb_out, std::uint32_t cb_length_t, std::uint32_t blk);
...
void apply_fused_scale_mask(
    ...
    for (std::uint32_t cur_blk = 0; cur_blk < cb_length_t; cur_blk += blk) {
...
void exp_cb(std::uint32_t cb_in, std::uint32_t cb_out, std::uint32_t cb_max, const std::uint32_t cb_length_t, std::uint32_t blk) {
    // requirements:
    //   cb_length_t of cb_in and cb_out are the same.
    //   blk is a divisor of cb_length_t
    ...
    compute_kernel_lib::reduce<reduce_type, ReduceDim::REDUCE_ROW, cb_in_id, cb_scaler_id, cb_out_id>(
        ...
        [use_prev_reduce](std::uint32_t) {
            if (use_prev_reduce) {
                ...
                DataflowBuffer(cb_prev_out_id).wait_front(1);
                ...
                DataflowBuffer(cb_prev_out_id).pop_front(1);
            }
            // If !use_prev_reduce, lambda is no-op (compiles away)
        });
}

template <PoolType reduce_type, std::uint32_t cb_in_id, std::uint32_t cb_scaler_id, std::uint32_t cb_ping_a, std::uint32_t cb_ping_b>
ALWI void reduce_cb_pass(std::uint32_t cur_pass, bool use_prev_reduce, std::uint32_t cb_length_t) {
    if ((cur_pass & 1) == 0) {
...
void kernel_main() {
    ...
    const std::uint32_t cb_length_t = get_arg(args::cb_length);
    ...
    constexpr auto cb_sumexps = dfb::recip_sum_exps;
    constexpr auto cb_prev_reduce = dfb::prev_reduce;
    constexpr auto cb_in0 = dfb::in0;
    constexpr auto cb_out0 = dfb::out0;
    constexpr auto cb_x = dfb::x;
    constexpr auto cb_recip = dfb::recip;
    constexpr auto cb_prev_max = dfb::prev_max;
    constexpr auto cb_mask_padded = dfb::mask_padded;
#if FUSED_SCALE_MASK
    ...
    std::uint32_t num_cb_passes = 1 + ((Wt - 1) / cb_length_t);  // ceiling divide
    ...
    const std::uint32_t cb_max_final = (num_cb_passes & 1) ? (std::uint32_t)cb_max : (std::uint32_t)cb_prev_max;
#else
    // cb_max is only consumed on the numeric-stable path; exp_cb ignores its cb_max argument otherwise. Bind
    // the unused slot to a valid DFB (cb_exps) so the handle resolves without cb_max (c_8) being allocated.
    const std::uint32_t cb_max_final = cb_exps;
#endif
    const std::uint32_t cb_sum_final = (num_cb_passes & 1) ? (std::uint32_t)cb_sumexps : (std::uint32_t)cb_prev_reduce;
    ...
    for (std::uint32_t ncht = 0; ncht < NCHt; ncht++) {
        ...
        std::uint32_t cur_cb_length_t = cb_length_t;
#ifdef NUMERIC_STABLE
        /*
         * --------------------------------------------------------
         * --------------------------------------------------------
         ...
         * --------------------------------------------------------
         */
        for (std::uint32_t cur_pass = 0; cur_pass < num_cb_passes; cur_pass++) {
