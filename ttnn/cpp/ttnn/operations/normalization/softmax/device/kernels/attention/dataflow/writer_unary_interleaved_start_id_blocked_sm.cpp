...
void kernel_main() {
    ...
    constexpr auto cb_id_out0 = dfb::out0;
    constexpr std::uint32_t onetile = 1;

    constexpr auto cb_id_mask = dfb::mask_padded;
    ...
    const std::uint32_t tile_bytes = cb_id_out0_obj.get_entry_size();
    ...
    if (mask_padded_data) {
        ...
        std::uint32_t ptr = (cb_id_mask_obj.get_write_ptr());
        // same pointer, but for zeroing out the tile
        volatile tt_l1_ptr std::uint16_t* zero_ptr =
            reinterpret_cast<volatile tt_l1_ptr std::uint16_t*>(cb_id_mask_obj.get_write_ptr());
