...
**PORTED (7 of 7 factories) — COMPLETE.** All seven factories are converted to `MetalV2FactoryConcept`
(`create_program_artifacts`) and pass on device: the five **General** factories
(`SoftmaxProgramFactoryGeneral{WSmall,HSmall,HLarge,CLarge,WLarge}`) plus both **Attention** factories
(`SoftmaxProgramFactoryAttentionOptimized` interleaved, `SoftmaxShardedProgramFactoryAttentionOptimized`
sharded). The General five were co-ported with `moreh/moreh_softmax` as the shared-kernel port-together set
(issue #51081); the two attention factories own their own kernels and were ported in later passes. The
`program_factory_t` variant is now all-Metal-2.0; `AllFactoriesValid` satisfied.
...
Both Attention factories are device-verified on wormhole_b0 via `fused/test_softmax.py` =
373 passed / 1 skipped / 0 failed — the full scale-mask / causal / numeric-stable / mask-padded /
...
- **Sharded Attention factory — DONE (this pass).** `SoftmaxShardedProgramFactoryAttentionOptimized` ported
  to `create_program_artifacts` (compute `softmax_sharded.cpp` + 3 runtime-selected readers
  `reader_unary_sharded_sm{,_causal_mask_hw_dims,_rm_mask}.cpp`; no writer). Device-verified via the sharded
  cases in `fused/test_softmax.py::test_softmax_sharded_stable_with_program_cache`. Key decisions:
  - **Borrowed-memory DFBs (`borrowed_from`)** for `c_0` input (← SRC) and `c_11` output (← DST), each a
    compute **self-loop** (single-toucher borrowed CB). First real use of the borrowed-input self-loop; works.
  - **`c_3` (attn mask) flips by config:** borrowed-from-MASK + compute self-loop when `SHARDED_CAUSAL_MASK`
    (sharded-resident mask; the reader neither binds nor reads it); reader-PRODUCER + compute-CONSUMER (1P+1C,
    not borrowed) when the mask is interleaved / row-major / hw-dims. The reader's mask-read block is
    `#ifndef SHARDED_CAUSAL_MASK`-gated to match. The borrowed condition is keyed on `SHARDED_CAUSAL_MASK`
    (`is_causal && mask.is_sharded()`), i.e. it assumes a sharded mask is always causal — the audit's model;
    a sharded+non-causal mask (if it ever exists) would need revisiting.
  - **`out0` self-loop needs its own `unpack_modes` entry under fp32** — unlike the interleaved factory where
    `out0` is producer-only, here (no writer) the compute is also declared CONSUMER of `out0`, so a Float32
    `out0` requires an explicit `unpack_modes` entry. Missing it was the one failure caught on device
    (`test_softmax_sd`, fp32); fixed. General lesson: every self-loop Float32 DFB the compute touches (incl.
    borrowed output) needs an `unpack_modes` entry, not just the obvious inputs.
  - Named CTAs cleaned up the 3 readers' intricate positional CTA layouts; `numeric_stable` gates `c_9`/`c_10`.
...
- **Device tests:** `scripts/run_safe_pytest.sh --run-all tests/ttnn/unit_tests/operations/fused/test_softmax.py`
  → **373 passed, 1 skipped, 0 failed** (covers the ported General path — 3D/5D/multi-dim softmax — **and both
  now-ported Attention paths** — interleaved: scale-mask, causal, numeric-stable, mask-padded, large-kernel, fp32;
  sharded: `test_softmax_sharded_stable_with_program_cache` bf16/bf8/fp32 × scale-mask × numeric-stable; no JIT
  failures). The moreh nightly test (which shares the ported kernels,
