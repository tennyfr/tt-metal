...
- **General W-large CAPITULATED post-verification** — reverted to legacy `descriptor`. Its shared compute kernel (`moreh_softmax_w_large.cpp`) fails to JIT-compile in the fp32 path (out-of-scope LLK addrmod cliff; confirmed a port regression). See `METAL2_PORT_REPORT.md` and the moreh op's report. So the ported General set is **W-small, H-small, H-large, C-large** (4 of 5); W-large + the 2 attention factories remain on legacy.
