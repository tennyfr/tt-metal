...
- **W-large unit CAPITULATED post-verification.** The Metal 2.0 port of `moreh_softmax_w_large.cpp` compiled on the host but failed to JIT-compile in the `fp32_dest_acc_en` path — an out-of-scope LLK addrmod `impossible constraint in 'asm'` cliff (confirmed a port regression vs legacy). The w_large unit (both factories + 3 kernels + header decls) was reverted to legacy `descriptor`. See `METAL2_PORT_REPORT.md` → Handoff points. The four other units (W-small, H-small, H-large, C-large) are ported and pass on device.
