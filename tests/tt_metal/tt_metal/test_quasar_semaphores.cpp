...
static void run_snake_chain(
    ...
    const std::string src = OVERRIDE_KERNEL_PREFIX;
    ...
    for (uint32_t i = 0; i < N; ++i) {
        ...
        KernelSpec reader{
            .unique_id = READER,
            .source = src + "tests/tt_metal/tt_metal/test_kernels/dataflow/dram_to_l1_pipeline.cpp",
            .num_threads = 1,
        ...
        spec.kernels.push_back(KernelSpec{
            .unique_id = XFORM,
            .source = src + "tests/tt_metal/tt_metal/test_kernels/dataflow/transform_pipeline.cpp",
            .num_threads = 1,
            .compiler_options = {.defines = {{"INCOMING_SEM", "1"}, {"OUTGOING_SEM", "1"}}},
            .semaphore_bindings =
        ...
        KernelSpec writer{
            .unique_id = WRITER,
            .source = src + "tests/tt_metal/tt_metal/test_kernels/dataflow/l1_to_dram_pipeline.cpp",
            .num_threads = 1,
            .hw_config = experimental::DataMovementGen2Config{}};
...
TEST_F(QuasarMeshDeviceSingleCardFixture, FanInGather) {
    ...
    // Seed each node's own DRAM src slot with its coord-unique value.
    for (uint32_t y = 0; y < g.y; ++y) {
        for (uint32_t x = 0; x < g.x; ++x) {
    ...
    const experimental::NodeRange all_nodes(experimental::NodeCoord{0, 0}, experimental::NodeCoord{g.x - 1, g.y - 1});
    const std::string ksrc = OVERRIDE_KERNEL_PREFIX;
    experimental::ProgramSpec spec{.name = "fanin"};
    spec.semaphores.push_back(experimental::SemaphoreSpec{.unique_id = SEM, .target_nodes = all_nodes});
    spec.kernels.push_back(experimental::KernelSpec{
        .unique_id = READER,
        .source = ksrc + "tests/tt_metal/tt_metal/test_kernels/dataflow/dram_to_l1_pipeline.cpp",
        .num_threads = 1,
        .semaphore_bindings = {{.semaphore_spec_name = SEM, .accessor_name = "sem"}},
        .runtime_arg_schema = {.runtime_arg_names = {"dram_addr", "l1_addr", "num_elements", "dram_bank_id"}},
        .hw_config = experimental::DataMovementGen2Config{}});
    spec.kernels.push_back(experimental::KernelSpec{
        .unique_id = WRITER,
        .source = ksrc + "tests/tt_metal/tt_metal/test_kernels/dataflow/l1_to_dram_pipeline.cpp",
        .num_threads = 1,
        .semaphore_bindings = {{.semaphore_spec_name = SEM, .accessor_name = "sem"}},
        .runtime_arg_schema = {.runtime_arg_names = {"dram_addr", "l1_addr", "num_elements", "dram_bank_id"}},
        .hw_config = experimental::DataMovementGen2Config{}});
...
// Vertical analog of PerRowChains: each of the 8 columns is an independent top->bottom
// 4-node chain, all 8 run concurrently. Exercises the vertical NoC paths (per-row only
// stressed horizontal hops) and wider 8-way concurrency vs the 4 row chains.
...
TEST_F(QuasarMeshDeviceSingleCardFixture, GridBarrierAllToOne) {
    ...
    std::cout << "[BARRIER] target released after " << r[0] << " signals, expected " << num_signalers
              << (r[0] == num_signalers ? "  PASS" : "  FAIL") << std::endl;
    EXPECT_EQ(r[0], num_signalers);
}
