...
def rotate_batch(items: list, repeat: int) -> list:
    """Rotate slot assignment so slot j receives source item (j + repeat) % N."""
    if not items:
        return []
    n = len(items)
    return [items[(slot + repeat) % n] for slot in range(n)]


def restore_rotated_batch(slot_outputs: list, repeat: int) -> list:
    """Undo rotate_batch output back to source-item order."""
    if not slot_outputs:
        return []
    n = len(slot_outputs)
    restored = [None] * n
    for slot, output in enumerate(slot_outputs):
        restored[(slot + repeat) % n] = output
    return restored


def assert_repeat_batch_consistency(per_repeat_outputs: list[list[list[int]]]) -> None:
    """Assert repeated outputs are stable per source prompt."""
    assert len(per_repeat_outputs) >= 2, "repeat-batch comparison needs at least two repeats"
    expected_batch_size = len(per_repeat_outputs[0])
    for repeat, outputs in enumerate(per_repeat_outputs):
        assert (
            len(outputs) == expected_batch_size
        ), f"repeat-batch comparison expected {expected_batch_size} slots, got {len(outputs)} at repeat {repeat}"

    baseline = per_repeat_outputs[0]
    for repeat, outputs in enumerate(per_repeat_outputs[1:], start=1):
        for slot, (expected, actual) in enumerate(zip(baseline, outputs)):
            if expected != actual:
                raise AssertionError(f"repeat-batch comparison failed at repeat {repeat}, slot {slot}")


def run_repeat_batch_comparison(
    items: list,
    *,
    repeat_count: int,
    run_repeat,
    ...
    """Run rotated repeats, restore outputs to source order, clean every repeat, then compare."""
    restored_outputs = []
    for repeat in range(repeat_count):
        try:
            slot_outputs = run_repeat(rotate_batch(items, repeat), repeat)
        finally:
            if cleanup_repeat is not None:
                cleanup_repeat(repeat)
        restored_outputs.append(restore_rotated_batch(slot_outputs, repeat))

    assert_repeat_batch_consistency(restored_outputs)
