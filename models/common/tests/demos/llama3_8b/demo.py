...
from models.common.tests.demos.run_helpers import assert_no_special_tokens, run_perf_benchmark, run_teacher_forcing
...
PERF_NUM_DECODE_TOKENS = 200
EVAL_REPEAT_BATCHES = 3
_MESH_DEVICE_TO_SHAPE = {
    "N150": (1, 1),
    "N300": (1, 2),
    "T3K": (1, 8),
    "TG": (4, 8),
...
DEMO_CASES = {
    "token-accuracy": DemoCase("token-accuracy", batch_size=1, max_seq_len=DEFAULT_MAX_SEQ_LEN, num_decode_tokens=0),
    "batch-1": DemoCase(
        "batch-1",
        batch_size=1,
        max_seq_len=DEFAULT_MAX_SEQ_LEN,
        num_decode_tokens=PERF_NUM_DECODE_TOKENS,
    ...
    "eval-32": DemoCase("eval-32", batch_size=32, max_seq_len=DEFAULT_MAX_SEQ_LEN, num_decode_tokens=200),
...
def _hf_model_id() -> str:
    return os.environ.get("HF_MODEL", "meta-llama/Llama-3.1-8B-Instruct")


def _model_target_id() -> str:
    hf_model = _hf_model_id()
    if Path(hf_model).exists():
        return Path(hf_model).name
    return hf_model


def _max_num_blocks(max_batch_size: int, max_seq_len: int, block_size: int = DEFAULT_BLOCK_SIZE) -> int:
    return int(max_batch_size) * int(math.ceil(max_seq_len / block_size))


def _resolve_llama_head_counts(hf_model: str | None = None) -> tuple[int, int]:
    hf_model = hf_model or _hf_model_id()
    try:
...
def create_llama3_for_causal_lm(
    mesh_device, optimizations="performance", max_batch_size=32, max_seq_len=DEFAULT_MAX_SEQ_LEN
):
    ...
    max_num_blocks = _max_num_blocks(max_batch_size, max_seq_len, block_size)
...
def _mesh_device_shape_from_env() -> tuple[int, int]:
    env = os.environ.get("MESH_DEVICE", "").strip().upper()
    if not env:
        pytest.skip("MESH_DEVICE must be set to N150, N300, T3K, or TG.", allow_module_level=True)
    shape = _MESH_DEVICE_TO_SHAPE.get(env)
    if shape is None:
        pytest.skip(
            f"Unsupported MESH_DEVICE={env!r} for Llama 3.1 8B; use N150, N300, T3K, or TG.",
            allow_module_level=True,
        )
    return shape


def _device_params_from_env() -> dict:
    params = {TRACE_MODEL_KEY_PARAM: "llama3.1-8b", "num_command_queues": 1}
    env = os.environ.get("MESH_DEVICE", "").strip().upper()
    shape = _MESH_DEVICE_TO_SHAPE.get(env)
...
@pytest.mark.parametrize(
    "test_config",
    [pytest.param(case_name, id=case_name) for case_name in DEMO_CASES],
)
@pytest.mark.parametrize(
    "device_params",
    [_device_params_from_env()],
    indirect=True,
)
@pytest.mark.parametrize("optimizations", ["performance", "accuracy"])
def test_llama3_8b(test_config, mesh_device, device_params, optimizations):
    ...
    try:
        ...
        elif test_config in ("batch-1", "batch-32", "batch-32-ci"):
            _run_perf_benchmark(
                llm,
                mesh_device,
...
def _run_token_accuracy(llm, mesh_device, expected, optimizations: str):
    ...
    if os.environ.get("CI") == "true":
        ...
        if not central or "top1" not in central or "top5" not in central:
            raise ValueError(
                f"No centralized accuracy target for {_model_target_id()} on {get_device_name(mesh_device)} "
                f"(batch_size=1, seq_len={prompt_len}, workload=token-accuracy, "
                f"optimization_profile={optimizations}); add an active entry to models/model_targets.yaml."
            )
...
def _measure_teacher_forcing_accuracy(llm, mesh_device, *, optimizations: str, log_text=False):
    ...
    if os.environ.get("CI") == "true":
        ...
        benchmark_data.save_partial_run_json(
            ...
            ml_model_name=_model_target_id(),
            ml_model_type="llm",
            device_name=get_device_name(mesh_device),
...
def _run_perf_benchmark(llm, mesh_device, expected, batch_size, case_name, num_decode_tokens=None):
    ...
    default_decode_tokens = PERF_NUM_DECODE_TOKENS if num_decode_tokens is None else int(num_decode_tokens)
...
def _contiguous_page_table(max_batch_size: int, max_seq_len: int, *, repeat_per_lane: bool = False) -> torch.Tensor:
    max_num_blocks_per_user = math.ceil(max_seq_len / DEFAULT_BLOCK_SIZE)
    if repeat_per_lane:
...
def _eval_repeat_prompts_batch32() -> list[str]:
    return [f"Count upward from {i} and then write one short sentence about the number." for i in range(32)]
...
def _run_eval_repeat_batch32(llm) -> None:
    tokenizer = llm.tokenizer
    prompts = _eval_repeat_prompts_batch32()
    page_table = _contiguous_page_table(32, DEFAULT_MAX_SEQ_LEN)
    sampling_mode, sampling_params = _sampling_params_for_model(llm.model, case_name="eval-32")

    per_repeat = []
    for repeat in range(EVAL_REPEAT_BATCHES):
        executor = None
        try:
            executor = _build_demo_executor(
                llm,
            ...
            input_tokens, prompt_lens = preprocess_llama3_8b_chat_prompts(
                rotated_prompts,
                llm,
                reserve_decode_tokens=PERF_NUM_DECODE_TOKENS,
            )
            result = run_perf_benchmark(
                executor,
                tokens=input_tokens,
    ...
    assert not failures, f"eval-32 generated token IDs differed for users {failures[:10]}"
